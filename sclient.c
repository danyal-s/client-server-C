...
void* writer(void* arg) //This thread will write to the buffer, tbuffer
{
    while(1)
    {
		...
		while(charsct > 0)
		{
			...
			bufptr = bufptr + charsct;
			if((bufptr - buffer + 1) < BUFFER_SIZE)
			{
				bufptr++;
			}
			else
...
do
{
	...
	pthread_create(&t1, &tattrib, (void*)reader, &k);
	pthread_create(&t2, &tattrib, (void*)writer, &k);
